...
void videoInput::processPixels(unsigned char * src, unsigned char * dst, int width, int height, bool bRGB, bool bFlip) {
	...
	if (!bRGB) {
		...
		if (bFlip) {
			...
			/*Leon		for(int i = 0; i < numBytes; i+=3){
			*dst = *(src+2);
			dst++;
			...
			*/
			//Leon convert RAW10 to Monochrome
			srcShort = (unsigned short *)src;
...
int videoInput::start(int deviceID, videoDevice *VD) {
	...
	/* Leon : for RAW data, we don't want to do format conversion by the system
	AM_MEDIA_TYPE mt;
